...
<div class="group-settings blog configure">
    ...
    if (count($blogs) > 1) {
        ...
        ?>
        <p>
            Please select which blog you'd like to configure:
        </p>
        <?=form_open(null, 'method="GET"')?>
            <select name="blog_id" class="select2" id="blog-id" placeholder="Please select a blog">
                <option></option>
                <?php
            ...
            <noscript>
                <button type="submit" class="btn btn-xs btn-primary">Select</button>
            </noscript>
        <?=form_close()?>
        <?php
